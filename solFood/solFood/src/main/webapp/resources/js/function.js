...
function memberInsertCheckForm(memInsForm)
{
    ...
    if( (memInsForm.repasswd.value.length < 8) || (memInsForm.repasswd.value.length > 11)) {
        alert("\n비밀번호확인은 8자리에서 10자리를 입력하셔야 합니다.");
        memInsForm.repasswd.focus();
        return false;
    }
...
function memberUpDelCheckForm(memUpDelForm, selectVal)
{
	...
	if( (memUpDelForm.passwd.value.length < 8) || (memUpDelForm.passwd.value.length > 10)) {
		alert("비밀번호는 8자리에서 10자리를 입력하셔야 합니다.");
		memUpDelForm.passwd.focus();
		return false;
	}
	...
	if( (memUpDelForm.repasswd.value.length < 8) || (memUpDelForm.repasswd.value.length > 10)) {
		alert("비밀번호확인은 8자리에서 10자리를 입력하셔야 합니다.");
		memUpDelForm.repasswd.focus();
		return false;
	}
